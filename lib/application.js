    ...
    */
    ...
    for (var r in this.config.regex) {
      ...
      if (!protos.regex.validRegex.test(re)) {
        throw new Error(util.format("Bad regular expression in app.config.regex, where %s => %s\n\n\
%s\n", r, re, protos.i18n.badRegularExpression));
    ...
    var portStr = listenPort !== 80 ? ":" + listenPort : '';

    // Structure application's secure baseUrl
    var securePort = protos.config.bootstrap.server.useSSL ? protos.config.bootstrap.ssl.port : null;
    var securePortStr = securePort !== 443 ? ':' + securePort : '';

    this.baseUrl = "http://" + this.hostname + portStr;
    this.baseUrlSecure = "https://" + this.hostname + securePortStr;
...
Application.prototype.initialize = function() {
  ...
  // Handle async tasks
  this.once('after_init', function(self) {
  ...
  // Load controller handlers on app/handlers
  processControllerHandlers.call(this);
  
  // Load controllers on app/controllers
  loadControllers.call(this);
  
  // Emit 'after_init' event.
  this.emit('after_init', this);
...
/**
  ...
  The app behaves differently depending on the amount of parameters provided:
...
Application.prototype.data = function() {
  ...
  switch (args.length) {
    ...
    case 1:
      ...
      if (key.indexOf('.') > 0) {
        key = key.split('.');
        var out = this.__data, len = key.length, last = (len-1);
        for (var k,i=0; i < len; i++) {
          k = key[i];
          if (i === last) {
            return out[k];
...
function setupViewPartials() {
  ...
  for (var m in builtinHelper) {
    if (builtinHelper[m] instanceof Function) {
      self.registerViewHelper(m, m, builtinHelper);
    }
  }
...
function createControllerFunction(func) {
  ...
  var Hogan = protos.require('hogan.js');

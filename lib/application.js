...
/**
  Object containing the Date Objects representing the time for the job's next call

  @private
  @property __jobNextCallTime
  @type object
 */
  this.__jobNextCallTime = {};
 ...
 */
  ...
  for (i=0; i < this.views.static.length; i++) {
...
Application.prototype.addJob = function(job, callback, interval, runImmed) {
  ...
  var updateCallTime = function() {
    self.__jobNextCallTime[job] = new Date().valueOf() + interval;
  }
...
Application.prototype.removeJob = function(job) {
  ...
  if (job in state) {
    ...
    delete this.__jobNextCallTime[job];
...
/**
  Gets the date object in which a job will be called next
  
  @public
  @method getJobNextCallTime
  @param {string} job
  @return {object} Date object
 */

Application.prototype.getJobNextCallTime = function(job) {
  return this.__jobNextCallTime[job] || null;
}
...
Application.prototype.invalidateCacheInterval = function(spec) {
  ...
  var cacheID = (spec.cacheID instanceof Array) ? spec.cacheID : [spec.cacheID];
  ...
  this.addJob(job, function() {
    ...
    for (var key,len=cacheID.length,i=0; i < len; i++) {
      multi.delete(cacheID[i]);
    }
  ...
  // Log msg
  this[spec.nowarn ? 'debug' : 'log']("Invalidating Cache Key [%s] every %s ms", cacheKey, spec.interval);

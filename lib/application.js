 ...
 */
  ...
  for (r in this.config.regex) {
    ...
    re = this.config.regex[r];

    if (re.ignoreCase) {
      throw new Error(util.format("Detected case insensitive matching in app.config.regex['%s']: %s\n\n\
Protos doesn't allow this because this regex can't be used properly when matching routes, as regexes are\n\
merged to form one master route validation regex. JavaScript doesn't support partial case insensitive\n\
regex matching, which would be needed to allow case insensitive regexes in this situation.\n", r, re))
    }
...
Application.prototype.addAsyncTask = function() {
...
/**
  ...
  @private
  @method flushAsyncTask
 */

Application.prototype.flushAsyncTask = function() {

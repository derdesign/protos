      ...
      if (stat.isDirectory(dir)) {
        ...
        fs.readdirSync(dir).forEach(function(filename) {
          if (jsFile.test(filename)) {
            var callback = self.require(util.format('%s/%s/%s', relPath, dirname, filename));
            if (callback instanceof Function) {
              handlers[filename.replace(jsFile, '')] = callback;
            } else {
              throw new Error(util.format("Expected a function for handler: %s/%s", dirname, filename));
            }
          }
        });

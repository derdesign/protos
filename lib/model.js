...
Model.prototype.prepare = function(app) {
  ...
  for (var r in this.validation) {
    ...
    if (re instanceof RegExp) {
      ...
      re = this.validation[r];
      
      console.log(re);
      
      if (re.ignoreCase) {
        throw new Error(util.format("Detected case insensitive matching in %s.validation['%s']: %s\n\n\
In order to have a consistent interface, you should have the ability to use the same regular expressions\n\
for route and model validation. Route validation doesn't allow the case insensitive flag in regexes, since\n\
they can't partially match a string. \n\n\
This is the reason why it is enforced to have a consistent definition of regexes, so that they can be used\n\
in routes and models without any problem.\n", this.constructor.name, r, re))
      }

...
for (var i=0; i < httpMethods.length; i++) {
  ...
  (function(m) {
    ...
    routeFunction('raw_' + m,  function(route, arg2, arg3) {
      ...
      // Run registerRoute function with modified arguments
      registerRoute.apply({raw: true}, o.args);
    });
...
Controller.prototype.handler = function() {
  ...
  var name = (this instanceof Function) ? this.name : this.constructor.name;
  var alias = Controller.prototype.getAlias(name);
  var handlers = protos.app.handlers;
...
function staticViewExists(url) {
  var exists = false,
      staticViews = app.views.static,
      templateExtensions = app.templateExtensions;

  url = url.replace(app.regex.startOrEndSlash, '');

  for (var ext,i=0,len=templateExtensions.length; i < len; i++) {
    ext = templateExtensions[i];
